...
//    backgroudView_.backgroundColor = [UIColor colorWithRed:((float)((0xD71629 & 0xFF0000) >> 16))/255.0f green:((float)((0xD71629 & 0xFF00) >> 8))/255.0f blue:((float)(0xD71629 & 0xFF))/255.0f alpha:1.0f];
...
- (void)setBackgroundColor:(UIColor *)backgroundColor{
    backgroudView_.backgroundColor = backgroundColor;
}
...
//        float width = self.bounds.size.width/tabbarItems.count;
        float width = (self.bounds.size.width-100)/tabbarItems.count;
...
- (void)layoutTabbar{
    float width = (self.bounds.size.width-2*EdgeMargin)/self.tabbarItems.count;
    float height = self.bounds.size.height;
    float x = 0.0f;
    for (NSInteger i=0; i<self.tabbarItems.count; i++) {
        x = i*width+EdgeMargin;
    ...
    float trackX = width*self.selectedIndex+EdgeMargin;
    // trackX width
#warning _______
...
- (void)switchingFrom:(NSInteger)fromIndex to:(NSInteger)toIndex percent:(float)percent{
    ...
    if (toIndex > fromIndex) {
        trackX = width*fromIndex + width*percent + EdgeMargin;
    }
    else{
        trackX = width*fromIndex - width*percent + EdgeMargin;
    }
...
- (void)setSelectedIndex:(NSInteger)selectedIndex{
    if (_selectedIndex != selectedIndex) {
        ...
        float width = (self.bounds.size.width-2*EdgeMargin)/self.tabbarItems.count;
        float trackX = width*selectedIndex+EdgeMargin;
#warning ________
        trackView_.frame = CGRectMake(trackX, trackView_.frame.origin.y, CGRectGetWidth(trackView_.bounds), CGRectGetHeight(trackView_.bounds));
