...
- (void)commonInit{
    [self commonInitWithTabbarHeight:kDefaultTabbarHeight];
}

- (void)commonInitWithTabbarHeight:(CGFloat)hight{
    self.tabbarHeight = hight <= 0 ? kDefaultTabbarHeight : hight;
...
- (id)initWithFrame:(CGRect)frame andTabbarHeight:(CGFloat)height{
    if (self = [super initWithFrame:frame]) {
        [self commonInitWithTabbarHeight:height];
    }
    return self;
}
...
- (void)buildTabbar{
    ...
    tabbar_.backgroundColor = self.tabbarBackgroundColor;
