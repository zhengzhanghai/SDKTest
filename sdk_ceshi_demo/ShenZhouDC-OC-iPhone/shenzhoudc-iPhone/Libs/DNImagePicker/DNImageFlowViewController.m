...
- (BOOL)seletedAssets:(ALAsset *)asset
{
    ...
    if (self.selectedAssetsArray.count >= (self.maxSelected == 0? kDNImageFlowMaxSeletedNumber: self.maxSelected)) {
        
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedStringFromTable(@"alertTitle", @"DNImagePicker", nil) message:[NSString stringWithFormat:@"不能超过%ld张图片",(long)self.maxSelected] delegate:self cancelButtonTitle:NSLocalizedStringFromTable(@"alertButton", @"DNImagePicker", nil) otherButtonTitles:nil, nil];//NSLocalizedStringFromTable(@"alertContent", @"DNImagePicker", nil)
        [alert show];
