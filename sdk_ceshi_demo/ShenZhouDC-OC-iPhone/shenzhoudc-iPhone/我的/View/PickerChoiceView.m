...
@property(nonatomic,assign) NSInteger index;//选中的标题下标
...
+ (instancetype) shareInstanceIntiWithFrame:(CGRect)frame {
    if (picker == nil) {
        picker = [[PickerChoiceView alloc]initWithFrame:frame];
    }
    return picker;
}
...
- (void)setArrayType:(ARRAYTYPE)arrayType
{
    ...
    switch (arrayType) {
        case GenderArray:
        {
            self.selectLb.text = @"选择业务类型";
            [self.array addObject:@[@"安装",@"调试",@"巡检",@"故障处理",@"培训",@"售前交流",@"测试",@"其它"]];
        }
            break;
        case HeightArray:
        {
            self.selectLb.text = @"选择技术方向";
            [self.array addObject:@[@"网络类",@"安全类",@"服务器类",@"开发类",@"软件类",@"存储类",@"其它"]];
        }
            break;
        case weightArray:
        {
            self.selectLb.text = @"选择是否加班";
            [self.array addObject:@[@"加班",@"不加班"]];
        }
            break;
        case DeteArray:
        {
            self.selectLb.text = @"选择日期";
            [self creatDate];
        }
            break;
        case ProvinceArray:
        {
            self.selectLb.text = @"选择省份";
            [self.array addObject:_province];
        }
            break;
        case CityArray:
        {
            self.selectLb.text = @"选择城市";
            [self.array addObject:_city];
        }
            break;
        case RegionArray:
        {
            self.selectLb.text = @"选择区/县";
            [self.array addObject:_region];
        }
            break;
...
- (void)completeBtnClick{
    ...
    for (int i = 0; i < self.array.count; i++) {
        ...
        if (self.arrayType == DeteArray) {
            ...
            _index = [self.pickerV selectedRowInComponent:i]% arr.count;
            
        }else{
            
            NSString *str = [arr objectAtIndex:[self.pickerV selectedRowInComponent:i]];
            fullStr = [fullStr stringByAppendingString:str];
            _index = [self.pickerV selectedRowInComponent:i];
        }
        

    }
    
    [self.delegate PickerSelectorIndixString:fullStr AndIndex:_index];
